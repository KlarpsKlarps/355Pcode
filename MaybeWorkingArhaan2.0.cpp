...
/*basically, this code under just conficures the motors for driver control.*/
  pros::adi::DigitalOut mogyPISTY (DIGITAL_A);
  pros::MotorGroup left_wheels ({-1,-9});
  pros::MotorGroup right_wheels ({2,10});
  pros::Controller master (CONTROLLER_MASTER);
  pros::Controller controller (pros::E_CONTROLLER_MASTER);
  pros::Motor upperintake (6);
  pros::Motor lowerintake(7);
  while (true) {
	...
	if (master.get_digital(pros::E_CONTROLLER_DIGITAL_L2)){  //RUNS IT BAKWEdS
		lowerintake.move(-100);
		upperintake.move(100);
	}

	if (master.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_R1)) //oldmatics pushes out
		mogyPISTY.set_value(true);
  	}
	if (master.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_R1)){  //pushes oldmatics stick back inside the hole
